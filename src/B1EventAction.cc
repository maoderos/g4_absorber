...
void B1EventAction::EndOfEventAction(const G4Event* event)
{
  ...
  G4double P = particleGun->GetParticleMomentum()/GeV;
   ...
   for(G4int i=0; i < 2; i++) {
       ...
       std::ofstream mu_p_pos("plus" + std::to_string(i + 1) +  "/Energy" + momentum_z.str() + "_Angle_" + angle_b.str() + ".dat",std::ios_base::app);
       std::ofstream mu_m_pos("minus" + std::to_string(i + 1) +  "/Energy" + momentum_z.str() + "_Angle_" + angle_b.str() + ".dat",std::ios_base::app);
       ...
       if(HCE) {
          ...
          for(int i1 = 0; i1 < n_hit; i1++) {
           ...
           if (name == "mu+") {
              mu_p_pos << position.x()/(m) << "  " << position.y()/(m) << "  " << position.z()/(m) << " "  << energy/GeV << " " << momentum.x()/GeV << " " << momentum.y()/(GeV)<< " " <<  momentum.z()/(GeV) << " " << energy_k.str() << " " << angle_b.str() <<"\n";
             } else if (name == "mu-") {
               mu_m_pos << position.x()/(m) << "  " << position.y()/(m) << "  " << position.z()/(m) << " "  << energy/GeV << " " << momentum.x()/GeV << " " << momentum.y()/(GeV)<< " " <<  momentum.z()/(GeV) << " " << energy_k.str() << " " << angle_b.str() <<"\n";
