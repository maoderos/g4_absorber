   ...
   for(G4int i=0; i < 2; i++) {
       ...
       if(HCE) {
          ...
          for(int i1 = 0; i1 < n_hit; i1++) {
           ...
           if (name == "mu+") {
              mu_p_pos << position.x()/(m) << "  " << position.y()/(m) << "  " << position.z()/(m) << " "  << energy/GeV << " " << momentum.x()/GeV << " " << momentum.y()/(GeV)<< " " <<  momentum.z()/(GeV) << " " << energy_k.str() << " " << angle.str() <<"\n";
             } else if (name == "mu-") {
               mu_m_pos << position.x()/(m) << "  " << position.y()/(m) << "  " << position.z()/(m) << " "  << energy/GeV << " " << momentum.x()/GeV << " " << momentum.y()/(GeV)<< " " <<  momentum.z()/(GeV) << " " << energy_k.str() << " " << angle.str() <<"\n";
